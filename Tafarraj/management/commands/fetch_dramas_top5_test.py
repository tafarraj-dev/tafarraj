...
class Command(BaseCommand):
    help = "Test fetch: first 5 Korean dramas from aradramatv.cc"
    ...
    def handle(self, *args, **kwargs):
        ...
        for country_key, category_url in CATEGORY_URLS.items():
            ...
            self.stdout.write(f"  Found {len(links)} links — fetching details...\n")
            ...
            for link in links:
                try:
                    ...
                    if not data:
                        self.stdout.write(f"  SKIPPED (no data): {link}")
                        total_skipped += 1
                        continue

                    title = data.get("title") or data.get("title_arabic", "")
                    if not title:
                        self.stdout.write(f"  SKIPPED (no title): {link}")
                        total_skipped += 1
                        continue

                    # Use country from page, fallback to category country
                    ...
                    if created:
                        ...
                        self.stdout.write(f"    thumbnail : {data.get('thumbnail_url', 'MISSING')}")
                        self.stdout.write(f"    year      : {data.get('release_year', 'MISSING')}")
                        self.stdout.write(f"    episodes  : {data.get('total_episodes', 'MISSING')}")
                        self.stdout.write(f"    genres    : {data.get('genres_raw', 'MISSING')}")
                        self.stdout.write(f"    desc      : {str(data.get('description_arabic', 'MISSING'))[:100]}...")
                    ...
                    self.stdout.write(f"  ERROR: {link} — {e}")
                    total_skipped += 1
